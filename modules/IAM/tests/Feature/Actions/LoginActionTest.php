...
describe('LoginAction', function (): void {
    it('authenticates with valid credentials and returns token', function (): void {
        ...
        $result = resolve(LoginAction::class)->handle($payload, '127.0.0.1', 'TestAgent');
    ...
    it('throws for unknown email', function (): void {
        $payload = new LoginPayload(email: 'ghost@example.com', password: 'password123');

        expect(fn () => resolve(LoginAction::class)->handle($payload))
            ->toThrow(ValidationException::class);
    });

    it('throws for wrong password', function (): void {
    ...
    it('blocks users with a non-authenticatable status', function (string $state): void {
        $factory = match ($state) {
    ...
    it('allows pending users to authenticate', function (): void {
        ...
        $result = resolve(LoginAction::class)->handle($payload);
