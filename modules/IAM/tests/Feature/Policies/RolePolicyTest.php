...
describe('RolePolicy', function (): void {
    ...
    it('grants view and create purely by permission', function (): void {
        $policy = new RolePolicy;
        $actor = UserFactory::new()->createOne();
        $actor->givePermissionTo(PermissionEnum::RoleView->value, PermissionEnum::RoleCreate->value);

        expect($policy->view($actor))->toBeTrue()
            ->and($policy->create($actor))->toBeTrue();
    });

    it('blocks view and create without permission', function (): void {
        $policy = new RolePolicy;
    ...
    it('permits update/delete on normal roles with permission', function (): void {
        $policy = new RolePolicy;
    ...
    it('blocks update and delete on the super-admin role even with permission', function (): void {
        $policy = new RolePolicy;
        $editor = UserFactory::new()->createOne();
        $editor->givePermissionTo(PermissionEnum::RoleEdit->value, PermissionEnum::RoleDelete->value);
