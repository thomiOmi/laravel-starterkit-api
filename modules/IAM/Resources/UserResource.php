...
class UserResource extends JsonResource
{
    ...
    /**
     * @return array{id: string, name: ?string, email: ?string, avatar: ?string, status: ?string, status_label: ?string, roles: string[]|MissingValue, permissions: string[]|MissingValue, email_verified_at: ?string, created_at: ?string, updated_at: ?string, deleted_at: ?string}
     */
    public function toArray(Request $request): array
    {
        $attributes = $this->resource->getAttributes();

        $status = array_key_exists('status', $attributes) ? $this->resource->status : null;
