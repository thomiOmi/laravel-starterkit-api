...
3. Stubs must stay in sync with the module conventions: final readonly resource controllers returning `SuccessResponse` (no create), `declare(strict_types=1)` in every PHP stub, attribute-based models (`#[Fillable]`, `#[Hidden]`, `#[UseFactory]`) and commands (`#[Signature]`, `#[Description]`), `file_exists` guards in RouteServiceProvider, route name `api.{version}.{module}.` prefix (see console-commands rule)
4. New kit layers require a matching stub so the generator produces convention-compliant files
5. Generator path mapping (config/modules.php `paths.generator`): interfaces -> `app/Contracts` (namespace `Contracts`), emails -> `app/Mail` (namespace `Mail`), resource -> `app/Http/Resources` (namespace `Http/Resources`), command -> `app/Console/Commands` (namespace `Console/Commands`), lang -> `app/Lang`, helpers -> `app/Support` (namespace `Support`); scopes generate ke `app/Models/Scopes` (nwidart menurunkan namespace dari model path - JANGAN pindah path-nya). Casts, channels, classes, traits, component-class, dan replacement tetap default vendor (di luar anatomi)

## Declare inter-module dependencies in module.json requires
module.json carries an optional "requires" array listing dependency module names (TitleCase folder names, e.g. {"requires": ["IAM"]}). Validate with `php artisan module:check-dependencies` (app/Console/Commands/ModuleDependencyCheckCommand.php): it fails when a dependency is not an installed/enabled module or when the graph has a cycle. The core module (IAM) declares none. New modules get an empty "requires": [] scaffolded by the generator stub (stubs/module-generator/json.stub). Do not add requires for a module that is not registered in the config/modules.php allow-list.
