...
Module route files in `modules/{Module}/routes/` (e.g. `V1.php`), loaded by the module's own `RouteServiceProvider` (`modules/{Module}/app/Providers/RouteServiceProvider.php`) while the module is active. The module provider extends `Illuminate\Foundation\Support\Providers\RouteServiceProvider`, iterates `apiroute.supported_versions` (default `['V1']`), guards each file with `file_exists`, and mounts it on `api/{version}` with route name `api.{version}.{alias}.`. Routes live in the module; shared/global middleware belongs to the app.

## Rules

1. Base prefix `api/v1/{path}` (no module segment in the URL); route name `api.{version}.{module}.{name}` (e.g. `api.v1.iam.auth.register`)
...
4. Versioned files follow `V{number}.php` casing (V1.php, V2.php); new scaffold modules generate `routes/V1.php` directly (see console-commands rule)
...
Final URL `/api/v1/auth/register`, final name `api.v1.iam.auth.register`.