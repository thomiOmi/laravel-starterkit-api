...
Keep the custom `FeatureFlagMiddleware` instead of adopting the native Pennant middleware. The middleware resolves a flag in two ways, in order:
...
2. **Runtime**: any other name (or one absent from the registry) falls back to Pennant (`Feature::active`), e.g. the `beta-feature` placeholder defined in `AppServiceProvider` or a Pennant class in `app/Features/`.
