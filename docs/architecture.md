...
Proyek ini menggunakan arsitektur modular yang dirancang untuk mendukung multi-tenancy (SaaS), aplikasi web (SPA), dan aplikasi mobile.
...
sequenceDiagram
    ...
    participant Middleware as Middleware (Tenancy & Auth)
    participant Controller as Controller
    participant Action as Action
    participant Repo as Repository
    participant DB as Database

    Client->>Middleware: Request with X-Tenant header & Bearer Token
    Middleware->>Middleware: Initialize Tenant Context
...
- **Multi-tenancy:** Gunakan trait `HasTenant` pada model yang memerlukan isolasi data berdasarkan tenant.
